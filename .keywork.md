...
## 62. CẤU HÌNH LỊCH LÀM VIỆC & SLA TRONG CÀI ĐẶT HỆ THỐNG (DYNAMIC WORKING HOURS & SYSTEM SETTINGS) (v4.6.6)
1. **Lưu Trữ Động Bằng Bảng CSDL `system_settings`**:
   - Thay vì hardcode giờ làm việc trong mã nguồn, hệ thống lưu trữ tại CSDL PostgreSQL thông qua bảng `system_settings` với key `working_hours_config`.
   - Cung cấp 2 API chuẩn hóa:
     * `GET /api/v1/settings/working-hours`: Lấy cấu hình ca sáng (07:30 - 11:30), ca chiều (13:00 - 17:00), chế độ dừng SLA cuối tuần, và thưởng DPI khi làm ngoài giờ.
     * `PUT /api/v1/settings/working-hours`: Lưu cấu hình mới cho toàn trường.
2. **Giao Diện Trực Quan Tại Trang Cài Đặt (`settings.html`)**:
   - Thêm tab "Lịch Làm Việc & SLA" trong Cài Đặt Hệ Thống (`settings.html`) cho phép Quản trị viên dễ dàng điều chỉnh giờ làm việc thực tế của trường khi chuyển mùa (mùa hè/mùa đông) hoặc thay đổi quy chế.
3. **Tự Động Nạp Cấu Hình Vào Động Cơ Thời Gian (`flow_engine.py`)**:
   - `flow_engine.py` tự động truy vấn CSDL để lấy giờ làm việc hiệu lực cho mọi phép tính Lead Time, Exec Time và DPI.

---

## 63. CHUẨN HÓA TYPOGRAPHY, ĐỒ THỊ XU HƯỚNG ĐA KỲ & THẺ TRẠNG THÁI PASTEL (EXECUTIVE REPORT STANDARDIZATION) (v4.6.7)
1. **Chuẩn Hóa Typography Đồng Đều Toàn Bộ Thẻ**:
   - Mọi tiêu đề thẻ đạt chuẩn `text-xs font-extrabold text-slate-800`.
   - Mọi con số định lượng header đạt `text-xs font-extrabold text-slate-600`.
   - Mọi dòng chú thích chân thẻ (footer) đạt chuẩn đồng bộ `text-[11px] text-slate-500 font-semibold pt-2.5 border-t border-slate-100 min-h-[36px] flex items-center`.
2. **Xóa Bỏ Triệt Để Từ Ngữ Rút Gọn Vô Nghĩa**:
   - Loại bỏ hoàn toàn: `"Cơ cấu tính chất: Đóng 100.0%"` $\rightarrow$ `"Chuẩn hóa 100% nhiệm vụ"`.
   - Loại bỏ hoàn toàn: `"Ma trận 4 cấp độ: Đóng 100.0%"` $\rightarrow$ `"Phân loại 100% mức độ"`.
   - Chuẩn hóa 5 trụ cột ở footer: `"25% Hạn chót • 15% Thời lượng • 25% Hoàn thành • 20% Chất lượng • 15% Phản hồi"`.
3. **Hệ Màu Nền Pastel Nổi Bật Cho 6 Thẻ Trạng Thái Vòng Đời**:
   - Nâng tỷ lệ màu nền từ `bg-xxx-50` (bị nhạt trắng) lên `bg-xxx-100/70` với viền `border-xxx-300/80` và thẻ Quá hạn `border-2 border-rose-400 bg-rose-100/80`.
   - Đảm bảo nổi rõ màu sắc trang nhã, phân biệt trực quan tức thì giữa 6 trạng thái nhưng không gây chói mắt.

---

## 64. CHUẨN HÓA CÂN ĐỐI THẺ ĐỒ THỊ XU HƯỚNG & LOẠI BỎ TRÙNG LẶP NÚT CHU KỲ (EXECUTIVE TREND CARD SYMMETRY & CLEAN HEADER) (v4.6.8)
1. **Loại Bỏ Hoàn Toàn Nút Trùng Lặp Trong Header Thẻ 2**:
   - Không đặt các nút bấm `[Tháng] [Quý] [Năm]` bên trong thẻ Xu hướng SPI vì thanh công cụ trên cùng (`#tasksKpiStripContainer`) đã có sẵn bộ chọn chu kỳ và dropdown chọn kỳ cụ thể.
   - Giữ tiêu đề thẻ thoáng đãng, không bị chèn ép hoặc co chữ (`truncate`). Góc phải hiển thị nhãn phạm vi đồng bộ với các thẻ Donut: `6 Tháng gần nhất`, `4 Quý gần nhất`, `Các năm học`.
2. **Cân Bằng Thị Giác Tuyệt Đối Với 3 Ô Thống Kê Xu Hướng Dưới Đồ Thị**:
   - Để đồ thị Line Chart không bị "quá nhỏ" hay "trống trải" so với các thẻ Donut tròn đầy bên cạnh, bổ sung dải 3 ô thống kê giá trị bên dưới:
     * Ô 1: `🎯 Mục tiêu: ≥ 80.0%` (Nền xanh lục pastel `bg-emerald-50/80`).
     * Ô 2: `📈 Điểm kỳ này: xx.x%` (Nền xanh tím pastel `bg-indigo-50/80`).
     * Ô 3: `🚀 Tăng trưởng: ▲ +xx.x%` (Nền tím pastel `bg-purple-50/80`).
   - Cấu trúc này tạo sự đồng bộ hoàn mỹ về nhịp điệu thị giác với Thẻ 3 (`≥70%, Đang làm, Rủi ro`), Thẻ 4 (`Chiến lược, Thường xuyên, Đột xuất`) và Thẻ 5 (`Khẩn cấp, Cao, Trung bình, Thấp`).
3. **Đường Xu Hướng Liền Mạch Trọn Vẹn Cả Khung Hình (Continuous Institutional Baseline)**:
   - Khi các kỳ trong quá khứ chưa có dữ liệu giao việc số (trước ngày Go-Live), hệ thống đối sánh với mốc chuẩn lịch sử trường HueIC (`HISTORICAL_BASELINES`) để đường cong Bézier luôn trải dài liên tục từ cạnh trái sang cạnh phải, stroke dày `stroke-width="4"`, không bị đứt đoạn hay chỉ co cụm ở một góc.

---

## 65. CHUẨN HÓA KHỐI THEO DÕI THỜI GIAN NỀN SÁNG & THUẬT NGỮ QUẢN TRỊ ĐẠI HỌC (LIGHT-MODE TIME INTELLIGENCE & ACADEMIC MANAGEMENT TERMINOLOGY) (v4.6.9)
1. **Loại Bỏ Hoàn Toàn Giao Diện Tối & Kiểu Dáng Hacker/Cyberpunk**:
   - Nghiêm cấm sử dụng các khối nền đen tối xì (`bg-[#0b1120]`, `bg-slate-900`) hoặc hiệu ứng neon xanh/tím chói mắt trong hệ thống giao diện HueIC IMP.
   - Toàn bộ các khối báo cáo thời gian và hiệu suất điều hành phải sử dụng **Giao Diện Nền Sáng (Clean Light Mode Card)**: `bg-white border border-slate-200/90 rounded-2xl shadow-xs p-4.5`.
2. **5 Thẻ Đo Lường Pastel Tinh Tế (Light Micro-Cards Grid)**:
   - Các trạm đo lường được tổ chức thành 5 thẻ con pastel dịu mắt (`bg-xxx-50/60 border border-xxx-200/70`), phân biệt màu sắc rõ ràng (Xanh da trời, Tím chàm, Vàng cam, Xanh ngọc lục bảo, Tím Huế).
   - Số liệu thống kê đạt chuẩn font đen đậm tương phản cao `font-manrope font-black text-2xl text-xxx-950`, dễ nhìn và rõ nét trong mọi điều kiện ánh sáng văn phòng.
3. **Thuần Việt 100% Thuật Ngữ Theo Chuẩn Quản Trị Hành Chính Đại Học**:
   - Xóa bỏ triệt để các thuật ngữ mượn từ cơ học chất lỏng / công nghệ thông tin DevOps gây khó hiểu cho Ban Giám Hiệu và cán bộ quản lý:
     * Loại bỏ `"Bảng viễn trắc vận tốc & dòng chảy thời gian"` $\rightarrow$ Đổi thành: **`"THEO DÕI THỜI GIAN & TỐC ĐỘ XỬ LÝ CÔNG VIỆC"`** (kèm badge: `Chỉ số thời gian & SLA`).
     * `Lead Time` $\rightarrow$ **`Tổng thời gian xử lý`** (*Từ giao việc đến hoàn thành*).
   ...
   - Quy chuẩn thời gian hiển thị thân thiện: `8h/ngày (07:30-11:30 & 13:00-17:00)` • `Nghỉ T7 & CN (Không tính vào hạn xử lý)` • `Thưởng tiến độ khi xử lý ngoài giờ/CN`.
